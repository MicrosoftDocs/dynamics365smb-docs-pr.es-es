...
description: 'Este artículo describe las diversas formas de trabajar con IVA tanto manualmente como con la configuración automática, para ayudarle a cumplir las regulaciones específicas del país o región.'
author: brentholtorf
ms.author: bholtorf
...
## <a name="calculating-and-displaying-vat-amounts-on-sales-and-purchase-documents"></a>Calcular y mostrar los importes del IVA en documentos de ventas y compras
...
Puede calcular y visualizar de forma diferente los importes del IVA en los documentos de venta y de compra. La diferencia depende del tipo de cliente o proveedor con el que esté tratando. También se puede cambiar el importe del IVA calculado de forma manual, por ejemplo, para que coincida con el calculado por el proveedor en una transacción concreta.

### <a name="including-or-excluding-vat-in-prices-and-line-amounts"></a>Inclusión o exclusión del IVA en precios e importes de línea

Si elige la casilla **Precios IVA incluido** en un documento de ventas, los campos **Precio de venta** e **Importe de línea** incluyen el IVA. De forma predeterminada, los valores de estos campos no incluyen el IVA. Los nombres de los campos reflejan si los precios incluyen IVA.  

Puede configurar el valor predeterminado de **Precios IVA incluido** de todos los documentos de venta de un cliente en el campo **Precios IVA incluido** de la ficha **Cliente**. También puede configurar los precios de los productos con el IVA incluido o no incluido. Por lo general, los precios en la Ficha de producto excluirán el IVA.
...
#### <a name="using-vat-business-posting-groups-and-customer-price-groups"></a>Usar Grupos contables de IVA de negocio y grupos de precio de cliente
...
* Para usar un tipo de IVA para clientes específicos, puede elegir un grupo en el campo **Grupo de registro de IVA de negocio (precio)** en la página **Grupo precio cliente**. Por ejemplo, esta configuración resulta útil cuando desea que se aplique un precio a todos los clientes en una determinada región geográfica o una industria concreta.
...
### <a name="modifying-the-vat-date-in-posted-entries"></a>Modificar la fecha de IVA en los movimientos registrados
...
## <a name="correcting-vat-amounts-manually-on-sales-and-purchase-documents"></a>Corrección manual de los importes del IVA en los documentos de ventas y compras
...
Para habilitar los cambios manuales del IVA en los documentos de venta, siga estos pasos. Los pasos son similares a los de la página **Configuración de compras y pagos**.
...
También puede ajustar los importes de IVA en general, ventas y diarios de compras. Por ejemplo, podría ser necesario realiza run ajuste si introduce una factura de proveedor en el diario y hay una diferencia entre el importe del IVA calculado por [!INCLUDE[prod_short](includes/prod_short.md)] y el de la factura del proveedor.  

### <a name="to-set-the-system-up-for-manual-vat-entry-in-general-journals"></a>Para configurar el sistema para el movimiento de IVA manual en un diario general
...
3. Cuando haya completado la configuración, puede ajustar el campo **Importe IVA** de la línea del diario general, o el campo **Importe IVA contrap.** de la línea del diario de ventas o compras. [!INCLUDE[prod_short](includes/prod_short.md)] comprobará que la diferencia no sea mayor que el máximo especificado.  
...
7. Cree una combinación **Grupo contable negocio** para la autoridad de IVA y **Grupo contable producto** para el IVA de importación. Para esta combinación, en el campo **Cuenta de compras**, elija la cuenta contable de IVA de importación.  

### <a name="to-create-a-new-invoice-for-the-import-authority-vendor-after-you-complete-the-setup"></a>Para crear una factura nueva para la autoridad de importación una vez haya finalizado la configuración
...
## <a name="processing-certificates-of-supply"></a>Procesamiento de certificados de oferta
...
4. Para incluir la información de las líneas en el documento de envío en el certificado, active **Imprimir detalles de línea**.  
5. Para que [!INCLUDE[prod_short](includes/prod_short.md)] cree certificados para los envíos contabilizados que no disponían de uno, active el botón de alternancia **Crear certificados de suministro si no se han creado**. Cuando active el botón de alternancia, se crearán los nuevos certificados para todos los envíos registrados que no tengan certificados dentro del rango seleccionado.  
...
3. En el campo **Estado**, seleccione la opción correspondiente.  
   ...
   Si el cliente no devuelve el certificado de suministro firmado, elija **No recibido**. Deberá entonces enviar al cliente una nueva factura que incluya el IVA, porque la autoridad fiscal no aceptará la factura original.  

Para ver un grupo de certificados, desde la página **Certificados de suministro** actualice la información sobre el estado de los certificados pendientes a medida que los reciba de sus clientes. La información actualizada puede ser útil para buscar todos los certificados con un determinado estado, por ejemplo, **Requerido**, cuyo estado desee actualizar a **No recibido**.  

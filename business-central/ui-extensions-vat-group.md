...
1. Cree la configuración en [Configuración de Microsoft Entra ID para los miembros del grupo](#microsoft-entra-id-setup-for-group-members).
...
3. Crear usuarios que los miembros del grupo de IVA utilicen para autenticarse cuando se conecten al representante del grupo de IVA de [!INCLUDE[prod_short](includes/prod_short.md)]. Los usuarios deben tener licencias de usuario completas para [!INCLUDE[prod_short](includes/prod_short.md)].
...
Los miembros del grupo de IVA que tienen [!INCLUDE[prod_short](includes/prod_short.md)] local deben configurar un registro de aplicación en Microsoft Entra ID para el suscriptor de [!INCLUDE[prod_short](includes/prod_short.md)] del representante del grupo de IVA. El registro de la aplicación permite que el [!INCLUDE[prod_short](includes/prod_short.md)] en línea del representante del grupo de IVA autentique al miembro del grupo. Obtenga más información en [Inicio rápido: registrar una aplicación con la plataforma de identidad de Microsoft](/azure/active-directory/develop/quickstart-register-app).

Cuando el administrador del miembro del grupo de IVA crea el registro de la aplicación en Microsoft Entra ID, debe especificar la siguiente información.
...
* En la sección **Autenticación**, en la opción de seleccionar **Tipos de cuenta admitidos**, seleccione **Cuentas en cualquier directorio organizacional (Cualquier directorio de Microsoft Entra, de varios suscriptores)**.
...
* En la sección **Información general**, observe el **Id. de la aplicación (cliente)**. Los miembros del grupo de IVA necesitan el Id. al establecer la conexión con el representante del grupo.
...
3. En el campo **Cliente Secreto**, especifique el cliente secreto del registro de su aplicación en Microsoft Entra ID.
...
8. El campo **Miembros aprobados** muestra el número de miembros del grupo que están configurados para enviar declaraciones de IVA al representante del grupo. Para agregar nuevos miembros, elija el número para abrir la página de **Miembros aprobados del grupo de IVA** y agregue la siguiente información:
    ...
    3. En el campo **Empresa**, especifique la empresa de la que el miembro del grupo presenta las devoluciones de IVA en [!INCLUDE[prod_short](includes/prod_short.md)], como **CRONUS UK Ltd**.
...
1. Inicia sesión en **Azure Portal** como administrador de autenticación.  
2. Vaya a **Microsoft Entra ID**.   
3. Navegue a **Usuarios** y luego seleccione el usuario que desea realizar una acción.  
4. Seleccione **Métodos de autenticación** en la parte superior de la página, seleccione **Requerir volver a registrar la autenticación multifactor**. 
5. Vuelva a Dynamics 365 Business Central y seleccione renovar el token desde **Configuración del informe de IVA**.  

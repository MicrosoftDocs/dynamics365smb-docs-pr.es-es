...
> [!NOTE]
> [!INCLUDE[prod_short](includes/prod_short.md)] también admite extensiones de impresora personalizadas que agregan aún más características de impresión. Por lo tanto, si se instalan extensiones de impresora personalizadas, su aplicación puede incluir características de impresión que no se describen en este artículo. 
...
- La extensión **Integración de Impresión universal** está instalada
    ...
    Esta extensión se publica e instala de forma predeterminada como parte de [!INCLUDE[prod_short](includes/prod_short.md)] Online y local.  Puede verificar si se está instalado en la página **Administración de extensiones**. Para obtener más información, consulte [Instalación y desinstalación de extensiones en Business Central](ui-extensions-install-uninstall.md).
- [!INCLUDE[prod_short](includes/prod_short.md)] local:
  ...
  - Una aplicación para Business Central está registrada en su suscriptor de Azure AD y [!INCLUDE[prod_short](includes/prod_short.md)]
      ...
      Como otros servicios de Azure que funcionan con [!INCLUDE[prod_short](includes/prod_short.md)], Impresión universal requiere un registro de aplicación para [!INCLUDE[prod_short](includes/prod_short.md)] en Azure Active Directory (Azure AD). El registro de la aplicación proporciona servicios de autenticación y autorización entre [!INCLUDE[prod_short](includes/prod_short.md)] y Impresión universal.
...
- Una suscripción/licencia de Impresión universal para su organización.
    ...
    Para obtener más información, consulte [Licencia de Impresión universal](/universal-print/fundamentals/universal-print-license).
...
2. Instale el conector de Impresión universal para registrar impresoras que no puedan comunicarse directamente con Impresión universal.
    ...
    La mayoría de las impresoras del mercado no pueden comunicarse directamente con Impresión universal. Tendrá que instalar el conector de Impresión universal para estas impresoras. Para obtener más información, consulte [Instalar el conector de Impresión universal](/universal-print/fundamentals/universal-print-connector-installation).

3. Registre sus impresoras en Impresión universal.

    El registro de una impresora hace que Impresión universaler tenga en cuenta la impresora.
    ...
    - Para otras impresoras, registre las impresoras usando el conector de Impresión universal. 
      ...
      Para obtener más información, consulte [Registro de impresora](/universal-print-connector-printer-registration).
...
4. Cambie las propiedades de la impresora (opcional).
    ...
    Para obtener más información, consulte [Administrar la configuración de metadatos de la impresora](/universal-print/fundamentals/universal-print-printer-property-settings).

5. Conceda permisos a los usuarios para las impresoras.

    Para obtener más información, consulte [Permisos de impresora](/universal-print/fundamentals/universal-print-printer-permissions#printer-permissions).

6. Comparta las impresoras.
    ...
    Para obtener más información, consulte [Compartir una impresora](/universal-print/fundamentals/universal-print-printer-permissions#share-a-printer).
...
7. Habilite la conversión de documentos.
    ...
    Para obtener más información, consulte [Información general de conversión de documentos](/universal-print/fundamentals/universal-print-document-conversion).
...
### <a name="add-universal-printer-printers-to-business-central"></a>Agregar impresoras de Impresión universal a Business Central

Una vez configuradas y compartidas las impresoras en Impresión universal, estará listo para usarlas en Business Central. Hay dos formas de agregar impresoras de Impresión universal. Puede agregar las impresoras todas a la vez o individualmente, una a una.

Agregar impresoras individualmente le permite configurar la misma impresora de Impresión universal en Business Central más de una vez. A continuación, para cada impresora agregada, puede cambiar la configuración de impresión, como la bandeja de papel, el tamaño y la orientación. De esta manera, puede configurar impresoras para diferentes informes y documentos que tienen requisitos de salida especiales.
...
3. Siga las instrucciones en pantalla.
    ...
    - Si eligió **Agregar una impresora de Impresión universal**, entonces aparece la página **Configuración de Impresión universal**. Rellen el campo **Nombre**, seleccione **...** al lado del campo **Imprimir recurso compartido en Impresión universal** para seleccionar la impresora de Impresión universal. Rellene los campos restantes según sea necesario. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)].
...
Una vez que se ha agregado una impresora, puede ver y cambiar su configuración desde la **Administración de impresoras**. Simplemente seleccione la impresora y, a continuación, elija **Editar configuración de impresora**. 
...
- La extensión **Enviar a impresora de correo electrónico** está instalada
    ...
    Esta extensión se instala por omisión. Para obtener información sobre la instalación de extensiones, consulte 
- La funcionalidad de correo electrónico está configurada.

   Para obtener más información, consulte [Configurar correo electrónico](admin-how-setup-email.md).

### <a name="add-an-email-printer"></a>Agregar una impresora de correo electrónico

La página **Administración de impresoras** le muestra las impresoras que están configuradas actualmente. La página también le brinda acceso a la página **Configuración** de cada impresora para editar una configuración existente o configurar una nueva impresora.
...
Tome todas las precauciones de privacidad necesarias, incluida la garantía de que la solución de impresión de correo electrónico haya configurado correctamente los permisos, la configuración de privacidad y las políticas de retención. Es su responsabilidad proporcionar una dirección de correo electrónico correcta, verificada y operativa. Para obtener más información, consulte la [Declaración de privacidad de Microsoft](https://privacy.microsoft.com/privacystatement).
...
La página **Administración de impresoras** le permite configurar una impresora como impresora predeterminada para todos los trabajos de impresión. Puede especificar la impresora como predeterminada solo para usted o para todos los usuarios.
...
> [!NOTE]
> Cuando imprime el informe en cuestión, puede seleccionar uno diferente usando el campo **Imprimir** en la página de solicitud.

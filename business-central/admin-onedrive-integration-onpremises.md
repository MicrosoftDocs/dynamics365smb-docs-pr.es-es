...
description: Obtenga información sobre cómo configurar Business Central en las instalaciones para que se integre con OneDrive para el trabajo o la escuela (anteriormente conocido como OneDrive para la empresa).
author: jswymer
ms.topic: conceptual
...
Este artículo explica cómo configurar la integración de OneDrive para el trabajo o la escuela (anteriormente conocido como OneDrive para la Empresa) con Business Central en las instalaciones. A diferencia de [!INCLUDE[prod_short](includes/prod_short.md)] en línea, la conexión entre Business Central y OneDrive no se configura automáticamente. Si la conexión no se configura, los usuarios no pueden usar las funciones para OneDrive.

Hay dos tareas que deben realizarse para configurar la integración de OneDrive.

- La primera tarea consiste en registrar una aplicación (app) en su inquilino de Microsoft Entra de su plan de Microsoft 365. La aplicación registrada se utiliza con fines de autenticación. Esta tarea normalmente se realiza en Azure Portal y en el cliente web de Business Central.
...
Para conocer los pasos detallados para completar este paso, consulte [Registrar una aplicación en Microsoft Entra ID](/dynamics365/business-central/dev-itpro/administration/register-app-azure#register-an-application-in-azure-active-directory) en la ayuda para desarrolladores y profesionales de TI.
...
5. Elija **Conexión de prueba** y espere los resultados.
   ...
   - Si la prueba falla, recibe un mensaje que describe el problema. Por lo general, el problema tiene que ver con la URL que proporcionó. Elija **Aceptar** para volver a la página **Configurar Business Central**, verifique la URL y vuelva a intentarlo.
...
Este cambio tiene implicaciones a la hora de pasar a la nueva integración de OneDrive, dependiendo de si ya hay una aplicación de Microsoft configurada en la configuración asistida de **Configurar sus cuentas de Microsoft Entra**. 
...
> [!TIP]
> En la página **Configuración de la conexión de SharePoint** y la configuración asistida **Configurar cuentas de Microsoft Entra**, la aplicación de Microsoft Entra se identifica con el **Id. del cliente**.

- Si la aplicación en **Configurar cuentas de Microsoft Entra** es diferente a la aplicación en **Configuración de la conexión de SharePoint**, la integración de OneDrive cambia para usar la aplicación en **Configurar cuentas de Microsoft Entra**.

   En **Configuración de OneDrive** mientras realiza el cambio, obtiene un mensaje similar al siguiente texto: 
  ...
  `NNNNNNNNN-NNNN-NNNN-NNNN-NNNNNNNNNNNN` representa el ID de cliente de la aplicación en **Configurar cuentas de Microsoft Entra** que se ha cambiado a la la integración de OneDrive. 

  > [!IMPORTANT]
  > Para que la nueva integración de OneDrive funcione después de hacer el cambio, debe otorgar permiso a la aplicación para la API de SharePoint en el portal de Azure. Puede realizar este paso antes o después de cambiar a la nueva configuración de OneDrive. Para obtener más información, consulte la sección [Registrar una aplicación en Microsoft Entra ID para la integración de OneDrive](#registerapp).

- Si la aplicación en **Configurar cuentas de Microsoft Entra** es la misma que la aplicación en **Configuración de la conexión de SharePoint**, la integración de OneDrive usa la misma aplicación que antes, excepto por la configuración en la configuración **Configurar cuentas de Microsoft Entra**.
...
- Si no hay ninguna aplicación configurada en la configuración **Configurar cuentas de Microsoft Entra**, la integración de OneDrive utiliza la misma aplicación que antes.

   La configuración asistida de **Configuración de OneDrive** copia la configuración de la aplicación en la configuración **Configurar las cuentas de Microsoft Entra**, por lo que se usa para otras integraciones que podrían configurarse más adelante.

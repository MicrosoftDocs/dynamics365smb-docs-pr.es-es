...
> [!NOTE]
> Si utiliza cualquiera de los métodos de amortización de saldo decreciente y desea ejecutar la amortización durante varios años, debe ejecutar la amortización de cada año por separado. Si ejecuta la amortización para todo el período desde la fecha de adquisición hasta el final del último año fiscal o el último período contable, es probable que los resultados sean incorrectos. Por ejemplo, es posible que desee ejecutarlo durante varios años si ha importado datos heredados y utiliza las fechas de adquisición reales para sus activos y desea ponerse al día con la amortización acumulada. Para los métodos de saldo decreciente, [!INCLUDE [prod_short](includes/prod_short.md)] calcula la amortización permitida por año, comenzando con el valor contable registrado para cada año. No puede realizar una amortización de varios años en un solo paso.
>
> El informe **Activo fijo - Valor proyectado** puede proyectar amortizaciones para períodos de varios años, lo que puede resultar confuso en comparación con los resultados que obtiene si ejecuta amortizaciones para varios años utilizando uno de los métodos de saldo decreciente. 
...
## <a name="duplicate-entries-to-other-depreciation-books"></a>Entradas duplicadas en otros libros de amortización
...
## <a name="manual-depreciation"></a>Amortización manual

...
1. Revise la primera página y elija **Siguiente** para abrir la página para descargar los complementos.
1. En la columna **Implementar**, seleccione la casilla de los complementos que desea implementar, y luego elija **Descargar y continuar**.

    Un archivo con el nombre *OutlookAddins.zip* se descarga en su dispositivo.

1. En este punto, ha terminado con el trabajo que necesita hacer en Business Central, por lo que puede elegir **Hecho**.
...
1. Vaya a la carpeta donde se descargó OutlookAddins.zip y extraiga los archivos **Contact Insights.xml** y **Document View.xml** del .zip a una carpeta de su elección.
...
1. Inicie sesión en Centro de administración de Microsoft 365, y luego vaya a [Aplicaciones integradas](https://go.microsoft.com/fwlink/?linkid=2163967).

1. Elija **Cargar aplicaciones personalizadas**.
1. En la página **Cargar aplicaciones para implementar**, establezca **Tipo de aplicación** en **Complemento de Office**.
1. Seleccione **Cargar archivo de manifiesto (.xml) desde el dispositivo** > **Elegir archivo**.
1. Seleccione uno de los archivos de complemento **Context Insights.xml** o **Documents.xml** que ha extraído anteriormente.
1. Siga las instrucciones para asignar usuarios e implementar el complemento.
1. Repita los pasos del 9 al 11 para el otro archivo de complemento si lo desea.

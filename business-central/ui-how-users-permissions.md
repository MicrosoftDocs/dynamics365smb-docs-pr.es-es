...
En este tema se describe cómo los administradores crean usuarios y definen quién puede iniciar sesión en [!INCLUDE[d365fin](includes/d365fin_md.md)], y qué permisos se conceden a los diferentes tipos de usuario según las licencias.
...
> [!NOTE]
> El proceso de administración de usuarios y licencias varía según si [!INCLUDE[d365fin](includes/d365fin_md.md)] se implementa en línea o localmente. Por ejemplo, en implementaciones en línea, puede administrar usuarios solo después de que se agreguen a [!INCLUDE[d365fin](includes/d365fin_md.md)] desde Office 365. En implementaciones locales, puede crear, editar y eliminar usuarios directamente.  
...
Para obtener más información, vea [Administración de Business Central Online](/dynamics365/business-central/dev-itpro/administration/tenant-administration) en la ayuda para desarrolladores y profesionales de TI.

Cuando se asigna una licencia de [!INCLUDE[d365fin](includes/d365fin_md.md)] a los usuarios en Office 365, se pueden importar en la página **Usuarios** en [!INCLUDE[d365fin](includes/d365fin_md.md)] mediante la acción **Obtener nuevos usuarios desde Office 365**.
...
2. Dependiendo de lo que quiera hacer, elija la acción **Obtenga nuevos usuarios de Office 365** o **Actualizar usuarios de Office 365**.

Los usuarios y la información de usuario nuevos de su suscripción a Office 365 se agregarán a la página **Usuarios** en [!INCLUDE[d365fin](includes/d365fin_md.md)]. Para obtener más información sobre la sincronización de la información de usuario con Office 365, vea [Sincronización con Office 365](ui-how-users-permissions.md#synchronization-with-office-365).
...
Cuando asigna una licencia para [!INCLUDE[d365fin](includes/d365fin_md.md)] a un usuario en Office 365, hay dos formas de crear el usuario en [!INCLUDE[d365fin](includes/d365fin_md.md)]. 

* El administrador puede agregar el usuario eligiendo la acción **Actualizar usuarios de Office 365** en la página **Usuarios**.
...
Si cambia la información del usuario en Office 365, puede actualizar [!INCLUDE[d365fin](includes/d365fin_md.md)] para reflejar el cambio. Según lo que desee actualizar, use una de las acciones en la página **Usuarios**. Las acciones se describen en las tres últimas columnas de la tabla siguiente.

|Qué pasa cuando:|Primer usuario, primer inicio de sesión|Obtener usuarios desde Office 365|Actualizar usuarios desde Office 365|Restaurar los grupos de usuarios predeterminados del usuario|Actualizar grupos de usuarios|
|-|-|-|-|-|-|
...
|Actualice el registro de usuario en función de la información real en Office 365 : Estado, Nombre completo, Correo electrónico de contacto, Correo electrónico de autenticación.<!--<br /><br />Codeunit "Azure AD   Graph User".UpdateUserFromAzureGraph-->|**X**|**X**|**X**|**X**| |
|Sincronice planes de usuario (licencias) con licencias y roles asignados en Office 365.<!--<br /><br />Codeunit "Azure AD   Graph User".UpdateUserPlans-->|**X**|**X**| |**X**|**X**|
...
> [!NOTE]
> Para implementaciones locales, un administrador puede especificar cómo autenticar las credenciales de usuario en la instancia de [!INCLUDE[server](includes/server.md)]. Cuando crea un usuario, proporciona el tipo de credencial que está utilizando.<br /><br />
> Para obtener más información, vea [Tipos de autenticación y credenciales](/dynamics365/business-central/dev-itpro/administration/users-credential-types) en la sección Administración del contenido para desarrolladores y profesionales de TI de [!INCLUDE[d365fin](includes/d365fin_md.md)].
